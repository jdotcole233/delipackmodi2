        ...
        if (customerPhoneNumber!.isEmpty || customerPassword!.isEmpty) {
            ...
            if (customerPhoneNumber!.count < 10 || customerPhoneNumber!.count > 10){
                ...
                print(customerLoginData)
                    ...
                    self.network!.networkRequestToServer(requestEndpoint: "customer_login", requestMethod: "POST", requestData: customerLoginData) { (result) in
                        switch result {
                        case .success(let user) :
                            if (user.success_cue == "Success") {
                                NetworkService(context: self).switchViewController(storyboardid: "Main", storyboardidentifier: "searchRiderViewController")
//                                self.performSegue(withIdentifier: "loginPayment", sender: self)
                                print("user back \(user.success_cue)")

                            } else if (user.success_cue == "Failed") {
                                DeliPackAlert(context: self, alertTitle: "Login Error", alertMessage: "Failed to log in try again later")
                                return
                            }
                        default:
                            break
...
//        signInButton.layer.cornerRadius = CGFloat(Int(bitPattern: 8))
        network = NetworkService(context: self)
    }
